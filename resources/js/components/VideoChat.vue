...
<div class="container">
  ...
  <div>
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">Chat with <span id="ChatWithName">{{ chatWithName }}</span></div>
            <div class="card-body">
                <div id="messageOutput">{{ message }}</div>
                <hr>
                <form id="chatForm" @submit.prevent="sendMessage">
                  <div class="form-group mb-3">
                    <input type="text" v-model="messageElement" />
                  </div>
                  <button type="submit">Send</button>
                </form>                    
                  <input type="file" id="fileInput" ref="fileInput" />
                  <button id="uploadButton" @click="uploadFile">Upload</button>
                    <button id="callButton" @click="placeVideoCall(selectedUserID, selectedUserName);">Video</button>
                    <div id="message"></div>
            </div>
        </div>
    </div>
